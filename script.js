...
function calculateCe(terrain, dimMin, dimMax, protected) {
    ...
    // Расчет по СП 20.13330.2016 п.10.7
...
function calculateMuForLanternRoof(angle, height, width, distance) {
    ...
    return {
        ...
        'Схема Б.3 (вариант 2)': {
            ...
            'описание': `Покрытие с фонарями по схеме Б.3 (вариант 2). Расчет коэффициентов для фонарей по формулам Б.2 СП 20.13330.2016: μ = 2h/b = ${mu1}, μ = 2h/l = ${mu2}`,
...
function calculateMuForHeightDrop(h, l1, l2) {
    ...
    return {
        'Схема Б.8 (вариант 1)': {
            ...
            'описание': `Покрытие с перепадом высоты по схеме Б.8 (вариант 1). h=${h}м, l1=${l1}м, l2=${l2}м. Расчет по формуле Б.5 СП 20.13330.2016: μ₁ = min(h/l1, 2) = ${mu1}, μ₂ = min(h/l2, 2) = ${mu2}, μ₃ = 2.0`,
            'применение': 'Для расчета прочности'
        },
        'Схема Б.8 (вариант 2)': {
            'μ₁ на верхнем покрытии': mu1,
            ...
            'описание': `Покрытие с перепадом высоты по схеме Б.8 (вариант 2). h=${h}м, l1=${l1}м, l2=${l2}м. Расчет по формуле Б.5 СП 20.13330.2016: μ₁ = min(h/l1, 2) = ${mu1}, μ₂ = min(h/l2, 2) = ${mu2}, μ₃ = 0.0`,
            'применение': 'Для расчета прочности'
        }
    };
}
...
function calculateMuForParapet(h, distance) {
    ...
    return {
        'Схема Б.13 (вариант а)': {
            'μ': mu,
            'описание': `Парапет по схеме Б.13 (вариант а). Высота парапета h=${h}м, расстояние от парапета ${distance}м. Расчет по п.Б.13 СП 20.13330.2016: μ = min(2h/${distance}, 3) = ${mu}`,
            'применение': 'Для расчета нагрузки у парапета'
        },
        'Схема Б.13 (вариант б)': {
            'μ': '3.0',
            'описание': `Парапет по схеме Б.13 (вариант б). Высота парапета h=${h}м. Принимается максимальное значение μ = 3.0 согласно п.Б.13 СП 20.13330.2016`,
            'применение': 'Для расчета нагрузки у парапета'
        }
    };
}
