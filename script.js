...
function calculateMuForHeightDrop(h, l1, l2) {
    // Расчет по п.Б.8 СП 20.13330.2016
    const mu1 = Math.min(h / l1, 2).toFixed(2);
    const mu2 = Math.min(h / l2, 2).toFixed(2);
    
    // Дополнительные ограничения согласно п.Б.8 д)
    const sg = sgValue;
    const mu3_max = Math.min(h / Math.sqrt(sg), 4).toFixed(2);
    
    return {
        'Схема Б.8 (вариант 1)': {
            'μ₁ на верхнем покрытии': mu1,
            'μ₂ на нижнем покрытии': mu2,
            'μ₃ в зоне перепада': `2.0 (max ${mu3_max})`,
            'описание': `Покрытие с перепадом высоты по схеме Б.8 (вариант 1). h=${h}м, l1=${l1}м, l2=${l2}м. ` +
                       `Расчет по формуле Б.5 СП 20.13330.2016: μ₁ = min(h/l1, 2) = ${mu1}, μ₂ = min(h/l2, 2) = ${mu2}, ` +
                       `μ₃ = 2.0 (не более h/√S₀ = ${mu3_max})`,
            'применение': 'Для расчета прочности'
        },
        'Схема Б.8 (вариант 2)': {
            'μ₁ на верхнем покрытии': mu1,
            ...
            'описание': `Покрытие с перепадом высоты по схеме Б.8 (вариант 2). h=${h}м, l1=${l1}м, l2=${l2}м. ` +
                       `Расчет по формуле Б.5 СП 20.13330.2016: μ₁ = min(h/l1, 2) = ${mu1}, μ₂ = min(h/l2, 2) = ${mu2}, μ₃ = 0.0`,
            'применение': 'Для расчета прочности'
        }
    };
}
