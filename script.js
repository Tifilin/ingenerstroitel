...
function calculateMuForSingleSlope(angle) {
    ...
    return {
        'Схема 1 (равномерная)': {
            'Вся площадь покрытия': mu.toFixed(2),
            'описание': `Односкатное покрытие с углом наклона ${angle}°. Расчет по п.Б.1 СП 20.13330.2016`,
...
function calculateMuForPitchedRoof(angle) {
    ...
    return {
        'Схема 1 (равномерная)': {
            'Скат 1 (μ)': mu.toFixed(2),
            'Скат 2 (μ)': mu.toFixed(2),
            'описание': `Двускатное покрытие с углом наклона ${angle}°. Равномерное распределение по п.Б.1 СП 20.13330.2016`,
            'применение': 'Для расчета прогибов'
        },
        'Схема 2 (неравномерная - вариант 1)': {
            'Скат 1 (μ₁)': (1.5 * mu).toFixed(2),
            'Скат 2 (μ₂)': (0.5 * mu).toFixed(2),
            'описание': `Двускатное покрытие с углом наклона ${angle}°. Неравномерное распределение по п.Б.1 СП 20.13330.2016`,
            'применение': 'Для расчета прочности (вариант 1)'
        },
        'Схема 3 (неравномерная - вариант 2)': {
            'Скат 1 (μ₁)': (0.5 * mu).toFixed(2),
            'Скат 2 (μ₂)': (1.5 * mu).toFixed(2),
            'описание': `Двускатное покрытие с углом наклона ${angle}°. Неравномерное распределение по п.Б.1 СП 20.13330.2016`,
...
function calculateMuForArchedRoof(angle, ratio) {
    ...
    return {
        'Схема 1 (равномерная)': {
            'Вся площадь покрытия': muArch.toFixed(2),
            'описание': `Сводчатое покрытие с углом ${angle}° и отношением f/l = ${ratio}. Расчет по п.Б.2 СП 20.13330.2016`,
            'применение': 'Для равномерного распределения'
        },
        'Схема 2 (неравномерная)': {
            'Зона 1 (участок 0-0.5l)': '0.5',
            'Зона 2 (участок 0.5l-l)': '1.0',
            'Зона 3 (участок l-1.5l)': '1.5',
            'Зона 4 (участок 1.5l-2l)': '1.0',
            'Зона 5 (участок 2l-2.5l)': '0.5',
            'описание': `Сводчатое покрытие с углом ${angle}° и отношением f/l = ${ratio}. Неравномерное распределение по п.Б.2 СП 20.13330.2016`,
...
function calculateMuForLanternRoof(angle, height, width, distance) {
    ...
    return {
        'Схема 1 (основное покрытие)': {
            'Зона A (между фонарями)': mu.toFixed(2),
            'Зона B (у фонарей)': Math.max(mu, mu2).toFixed(2),
            'Зона C (угловые зоны)': Math.max(mu, mu1, mu2).toFixed(2),
            'описание': `Покрытие с фонарями (h=${h}м, b=${b}м, l=${l}м). Расчет по п.Б.3 СП 20.13330.2016`,
            'применение': 'Для основного покрытия'
        },
        'Схема 2 (покрытие фонарей)': {
            'На покрытии фонаря': mu1,
            'У торцов фонаря': mu2,
            'описание': `Покрытие с фонарями (h=${h}м, b=${b}м, l=${l}м). Расчет коэффициентов для фонарей`,
...
function calculateMuForShedRoof(angle) {
    ...
    return {
        'Схема 1 (равномерная)': {
            ...
            'описание': `Шедовое покрытие с углом наклона ${angle}°. Равномерное распределение по п.Б.5 СП 20.13330.2016`,
            'применение': 'Для расчета прогибов'
        },
        'Схема 2 (неравномерная)': {
            'Нечетные скаты': (1.4 * mu).toFixed(2),
            'Четные скаты': (0.6 * mu).toFixed(2),
            'описание': `Шедовое покрытие с углом наклона ${angle}°. Неравномерное распределение по п.Б.5 СП 20.13330.2016`,
            'применение': 'Для расчета прочности'
        }
    };
}
...
function calculateMuForMultiPitchedRoof(angle) {
    ...
    return {
        'Схема 1 (равномерная)': {
            ...
            'описание': `Многопролетное двускатное покрытие с углом наклона ${angle}°. Равномерное распределение по п.Б.6 СП 20.13330.2016`,
            'применение': 'Для расчета прогибов'
        },
        'Схема 2 (неравномерная)': {
            'Крайние пролеты': (1.4 * mu).toFixed(2),
            'Средние пролеты': (0.6 * mu).toFixed(2),
            'описание': `Многопролетное двускатное покрытие с углом наклона ${angle}°. Неравномерное распределение по п.Б.6 СП 20.13330.2016`,
            'применение': 'Для расчета прочности'
        }
    };
}
...
function calculateMuForMultiArchedRoof(angle, ratio) {
    ...
    return {
        'Схема 1 (равномерная)': {
            'Все пролеты': muArch.toFixed(2),
            'описание': `Многопролетное сводчатое покрытие с углом ${angle}° и отношением f/l = ${ratio}. Равномерное распределение по п.Б.6 СП 20.13330.2016`,
            'применение': 'Для расчета прогибов'
        },
        'Схема 2 (неравномерная)': {
            'Крайние пролеты': (1.4 * muArch).toFixed(2),
            'Средние пролеты': (0.6 * muArch).toFixed(2),
            'описание': `Многопролетное сводчатое покрытие с углом ${angle}° и отношением f/l = ${ratio}. Неравномерное распределение по п.Б.6 СП 20.13330.2016`,
            'применение': 'Для расчета прочности'
        }
    };
}
...
function calculateMuForHeightDrop(h, l1, l2) {
    ...
    return {
        'Схема 1 (снеговой мешок)': {
            ...
            'Нижнее покрытие (участок 2h)': mu2,
            'Остальная площадь верхнего покрытия': '1.0',
            'Остальная площадь нижнего покрытия': '1.0',
            'описание': `Покрытие с перепадом высоты h=${h}м. Образование снегового мешка по п.Б.8 СП 20.13330.2016. Расчет: m = min(2h, 8) = ${m}, μ₁ = min(m, 4) = ${mu1}, μ₂ = min(m/1.4, 4) = ${mu2}`,
...
function calculateMuForPointedRoof(angle) {
    ...
    return {
        'Схема 1 (равномерная)': {
            'Вся площадь покрытия': mu.toFixed(2),
            'описание': `Стрельчатое покрытие с углом наклона ${angle}°. Расчет по п.Б.2 СП 20.13330.2016`,
            'применение': 'Для равномерного распределения'
        }
    };
}
